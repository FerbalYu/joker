...
try {
  ...
  check('approval requests carry the provider ToolCall identity', Boolean(approvalA.toolCallId && approvalB.toolCallId), { toolCallA: approvalA.toolCallId, toolCallB: approvalB.toolCallId })
  ...
  const awaitingCardA = pageA.locator('[data-tool-card][data-tool-status="awaiting-approval"]').filter({ hasText: 'qa-approval-denied.txt' }).first()
  const awaitingCardB = pageB.locator('[data-tool-card][data-tool-status="awaiting-approval"]').filter({ hasText: 'qa-approval-denied.txt' }).first()
  await awaitingCardA.waitFor({ state: 'visible', timeout: 20_000 })
  await awaitingCardB.waitFor({ state: 'visible', timeout: 20_000 })
  check('window A ToolCard is awaiting approval rather than running', await awaitingCardA.locator('[data-tool-state-icon="awaiting-approval"]').count() === 1 && await awaitingCardA.locator('[data-tool-state-icon="running"]').count() === 0)
  check('window B ToolCard is awaiting approval rather than running', await awaitingCardB.locator('[data-tool-state-icon="awaiting-approval"]').count() === 1 && await awaitingCardB.locator('[data-tool-state-icon="running"]').count() === 0)
  await screenshot(pageA, 'window-a-awaiting-approval')
  await screenshot(pageB, 'window-b-awaiting-approval')
