...
- **P2 engineering boundary:** CI validates typecheck, minimal lint, unit tests, deterministic local integration contracts, coverage output/manifest, and Electron bundle build on Linux and Windows. No CI job calls real Provider/MCP/network services. Windows v0.1.0 → v0.1.1 install/upgrade/uninstall retention and packaged UI startup were separately validated in isolated directories. Linux AppImage/deb install, startup, session restoration, uninstall, and cleanup have separate Ubuntu 22.04 WSL2 evidence under `.qa/native-linux-wsl-20260729/`; that local evidence is not a hosted GitHub Actions artifact. macOS packaged startup remains unavailable in the current Windows environment, and hosted native-runner evidence still requires a repository remote and CI dispatch access.
